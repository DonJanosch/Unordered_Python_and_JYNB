...
class population:
    def __init__(self,ID,population_size_int,chromosome_template,mutation_rate = 0.05):
        ...
        self.extinct_selection_method = 'negated_fitness'
    ...
    def get_parent_id(self, list_of_exclusions = ''):
        ...
        creature_pool = self.get_all_creature_attributes(only_alive=True)
    ...
    def move_to_next_generation(self,number_of_children_to_generate_int=1):
        ...
        for _ in range(number_of_children_to_generate_int):
            ...
            if self.extinct_selection_method == 'negated_fitness':
                negated_fitness = list(self.get_all_creature_attributes(only_alive=True)['fitness'])
                negated_offset = max(negated_fitness)
                negated_fitness = [negated_offset - weight-1 for weight in negated_fitness]
                best_id = self.get_best_creature_attributes(only_id=True,only_alive=True)
                extinct_id = best_id
                while extinct_id==best_id:
                    extinct_id = np.random.choice(self.get_list_of_creatures_alive(),1,negated_fitness)
                #print('FN',list(self.get_all_creature_attributes(only_alive=True)['fitness']))
                #print('NP',negated_fitness)
                #print('ID',extinct_id)

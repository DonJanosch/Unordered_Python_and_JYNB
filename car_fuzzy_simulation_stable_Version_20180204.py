...
class simulation_enviroment:
    ...
    Input is the cars motor power which is consumed by air-friction and roller-friction. The difference goes into kinnetic energy
    ...
    The second cars motor power is controlled in a way that the car should mimic human behaviour and try always to stay
    at about 100% safety distance from the first car. Never the less it shal do better than just giving a PID controler the job to stay at 100% safety distance.
    ...
    def __init__(self):
        ...
        self.dt = 0.05 #simulation resolution in s (lower values make the controller act more often)
        ...
        self.look_up_Ppower1 = [[0,10,11,12,14,40,50 ,51 ,60,61,65,80,90 ,100], #Cars motor power look up table, seconds into simulation in s
                           [1,1 ,-2,-3,1 ,1 ,0.3,0.1,1 ,-3,-3,1 ,0.6,1  ]] #Cars motor power look up table, percentage of motorpower in 1 is 100%

        #Car 2 (the controled following car)
        ...
        self.smart_clipping = True

        #PID Controller
        self.K_p = 3 #Gain of the proportional part
        self.K_i = 5 #Gain of the integral part, since the behavior of the integral part would be depending on the sampling rate, it is scaled with the sampling rate to remain the same :)
    ...
    def build_simulation_enviroment(self):
        ...
        print('Setting up simulation the enviroment')
        ...
        self.results = np.zeros((int(self.simulation_time/self.dt)+1,len(self.column_headers))) #Make an empty numpy array in the correct size

        #Pre-compute the time for all time steps that are taken
        self.t_precomputed=np.arange(0,self.simulation_time+self.dt,self.dt) #Pre-compute all time labels for easy access
    ...
    def show_plots(self):
        ...
        plt.title('Fuzzy Controler Diagram, skipped first view seconds for better readability')
        plt.grid(True)
        ax0.legend()
        ...
        plt.title('PID Controler Diagram')
        plt.grid(True)
        ax0.legend()
        ...
        plt.title('Speed Setpoint Diagram')
        plt.grid(True)
        ax0.legend()

...
import hashlib #require for SHA256
...
def sha256(message,encoding='utf-8'): #convienient wrapper to easily compute sha256 hashes and return hex-strings (NOTE: string is without leading '0x')
    m = hashlib.sha256() #Assign new SHA256 object
    m.update(str(message).encode(encoding)) #Feed it
    return m.hexdigest() #return the hex value of the SHA256-Hash
...
def calculate_seed_from_word(message):
    '''Takes a random seed phrase or word, calculates the SHA256 hash value of it,
    ...
    '''
    tmp = sha256(message) #Calculate the has of  the seed word or phrase. REMAKR: If we would only use one haseh value, we would only yield 55 digits in Base27 but we need 81 of them :)
    tmp2 = sha256(tmp) #Hash the hash value again
    sha_source = str(tmp) + str(tmp2) #Concatenate the both hash values
    seed = base27encode(sha_source)[:81] #Converte them to the Base27 letter-Alphabet and return the first 81 digits of it
    return seed
